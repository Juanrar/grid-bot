...
export class GrvtClient {
    ...
    _validateOrder(spec, size, price) {
        ...
        if (sizeNum < spec.minSize) {
            throw new Error(`Size ${size} < min_size ${spec.minSize} para ${spec.instrument}`)
        }
        const notional = sizeNum * priceNum
        if (notional < spec.minNotional) {
            throw new Error(`Notional $${notional.toFixed(2)} < min_notional $${spec.minNotional}`)
        }
    }
    ...
    async createOrder(req, allowMarket = false) {
        ...
        if (isMarket && !allowMarket) {
            throw new Error("Solo se permiten órdenes limit (usar allowMarket=true para casos especiales)")
        }
        ...
        const data = await this._authedRequest(`${TRADING_URL}/create_order`, payload)
    ...
    async cancelAllOrders(instrument) {
        ...
        if (instrument) body.instrument = instrument
        try {
            const data = await this._authedRequest(`${TRADING_URL}/cancel_all_orders`, body)
