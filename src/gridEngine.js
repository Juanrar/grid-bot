...
export class GridEngine {
  ...
  async placeInitialOrders() {
    ...
    if (this.config.direction === "long") {
      if (positionSize > 0) {
        ...
        try {
          ...
          if (
            msg.includes("Insufficient margin to create order") ||
            msg.includes('"code":2080')
          ) {
            initialInventoryReady = false;
            console.warn(
              "⚠️  Sin margen para compra inicial. Arranco solo con BUYs por debajo del precio.",
            );
          } else {
            throw err;
          }
    ...
    for (const level of this.levels) {
      ...
      try {
        ...
        console.error(
          `❌ Error colocando nivel ${level.index} @ $${level.price}: ${err.message}`,
        );
      }
    }
  ...
  async executeInitialPurchase(currentPrice) {
    ...
    if (notional < this.spec.minNotional) {
      console.log(
        `⚠️  Compra inicial omitida: notional $${notional.toFixed(2)} < min`,
      );
      return;
    }
    ...
    const ask = parseFloat(
      ticker.best_ask_price ?? ticker.best_ask ?? ticker.last_price,
    );
    const safePrice = Math.floor(ask * 1.001 * 100) / 100;
    const size = (Math.floor(totalQty * 100) / 100).toString();

    console.log(
      `💰 Compra inicial: BUY ${size} ${this.pair} @ ~$${safePrice} (IOC, notional $${notional.toFixed(2)})`,
    );
  ...
  // ── NIVEL 1: Cierre de Emergencia (Stop-Loss) ────────────────────

  async executeEmergencyStop(currentPrice) {
    console.log(
      `\n🚨 ALERTA: STOP LOSS ALCANZADO a $${currentPrice}! Ejecutando cierre de emergencia...`,
    );
    this.running = false;
    if (this.interval) clearInterval(this.interval);

    // 1. Cancelar todas las órdenes
    await this.client.cancelAllOrders(this.pair);

    // 2. Obtener posición actual
    const position = await this.client.getPosition(this.pair);
    const posSize = position
      ? parseFloat(position.size ?? position.balance ?? "0")
      : 0;

    // 3. Vender a mercado para cerrar inventario (con allowMarket = true)
    if (posSize > 0 && this.config.direction === "long") {
      console.log(
        `📉 Cerrando posición de ${posSize} a mercado para frenar pérdidas...`,
      );
      try {
        await this.client.createOrder(
          {
        ...
        console.error(
          `❌ Falló la venta a mercado del Stop-Loss: ${err.message}`,
        );
      }
    }

    console.log("💀 Bot apagado por seguridad.");
    process.exit(0);
  }

  // ── NIVEL 2: Trailing Grid (Seguir tendencia al alza) ────────────

  async recenterGridUpwards(currentPrice) {
    console.log(
      `\n📈 Precio superó límite superior ($${currentPrice}). Recentrando grid...`,
    );

    // Pausar monitor momentáneamente
    this.running = false;

    // 1. Limpiar órdenes y memoria
    await this.client.cancelAllOrders(this.pair);
    this.processedFills.clear();

    // 2. Desplazar el rango para que el precio actual sea el nuevo centro
    const range = this.config.upperPrice - this.config.lowerPrice;
    this.config.lowerPrice = Math.round((currentPrice - range / 2) * 100) / 100;
    this.config.upperPrice = Math.round((currentPrice + range / 2) * 100) / 100;

    console.log(
      `🔄 Nuevo rango calculado: $${this.config.lowerPrice} - $${this.config.upperPrice}`,
    );

    // 3. Recalcular y colocar niveles (aprovechando que placeInitialOrders es idempotente)
  ...
  async _placeGridOrder(level) {
    ...
    if (notional < this.spec.minNotional) {
      console.log(
        `⚠️  Skip nivel ${level.index}: notional $${notional.toFixed(2)} < min`,
      );
      return;
    }
    ...
    console.log(
      `📝 ${level.side.toUpperCase()} ${level.quantity} @ $${level.price} (nivel ${level.index})`,
    );
  }
  ...
  async monitor() {
    ...
    // --- NUEVO: Verificación de Stop-Loss ---
    if (
      this.config.stopLossPrice &&
      currentPrice <= this.config.stopLossPrice
    ) {
      await this.executeEmergencyStop(currentPrice);
      return; // Cortamos la ejecución de este ciclo
    }

    // --- NUEVO: Verificación de Trailing Grid (Escape por arriba) ---
    if (this.config.trailingGrid && currentPrice >= this.config.upperPrice) {
      await this.recenterGridUpwards(currentPrice);
      return; // Cortamos la ejecución para que el bot se reinicie
    }
    ...
    const gridStep =
      (this.config.upperPrice - this.config.lowerPrice) / this.config.numGrids;
    ...
    for (const level of this.levels) {
      ...
      if (!covered)
        uncovered.push({ level, dist: Math.abs(lp - currentPrice) });
    }
    ...
    if (uncovered.length > 1) {
      ...
      for (let i = 1; i < uncovered.length; i++) {
        ...
        try {
          ...
          console.error(
            `❌ Falló re-colocar @ $${level.price}: ${err.message}`,
          );
        }
      }
    }
  ...
  async _killDuplicatesAndOrphans(openOrders) {
    ...
    console.log(
      `🔴 ${openOrders.length} órdenes (esperadas ≤${expectedLevels.length}) — limpiando`,
    );
    ...
    const expectedPrices = new Set(
      expectedLevels.map((l) => l.price.toFixed(2)),
    );
  ...
  async _placeCounterOrders(filledLevels, currentPrice) {
    for (const level of filledLevels) {
      ...
      if (!counter) {
        console.log(
          `⚠️  Sin nivel contrario para index ${counterIndex}, omitido`,
        );
        continue;
      }
      ...
      try {
        ...
        console.error(
          `❌ Falló contra-orden @ $${counter.price}: ${err.message}`,
        );
      }
    }
  }
  ...
  async stop() {
    ...
    console.log(
      `✅ ${cancelled} órdenes canceladas. Posición (si hay) NO se cierra.`,
    );
  }
}
